...
def wlsFilter(IN, vlambda=None, alpha=None, L=None):
	"""
	...
	"""
	dy = np.diff(L, 1, 1)
	dy = -vlambda / (np.abs(dy) ** alpha + smallNum)
	dy = np.pad(dy, [0,1], 'constant')
	dy = np.reshape(dy, -1)

	dx = np.diff(L, 1, 2)
	dx = -vlambda / (np.abs(dx) ** alpha + smallNum)
